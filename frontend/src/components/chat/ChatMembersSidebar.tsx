                ...
                <div className="space-y-2">
                    ...
                    >
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all shadow-sm ${
                            activeChannel.type === 'PROJECT' ? 'bg-blue-600 text-white rotate-12' : 'bg-slate-200 text-slate-500'
                        }`}>
                        ...
                        {members.map(member => {
                            ...
                            return (
                                <button 
                                    ...
                                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-all duration-300 border group ${
                                        isActive 
                                ...
                                >
                                    <div className="relative shrink-0">
                                        ...
                                        <div className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-slate-50 group-hover:border-white transition-all ${
                                            online ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-slate-300'
                                        }`} />
                                    </div>
                                    <div className="flex flex-col min-w-0 items-start overflow-hidden flex-1">
                                        <span className={`text-[13px] truncate w-full text-left tracking-tight ${
                                            isActive ? 'font-bold text-blue-600' : 'font-bold text-slate-700'
                                        }`}>
                                            {member.firstName} {member.lastName}
                                        </span>
                                        <div className="flex items-center gap-1.5 overflow-hidden w-full">
                                            {member.role === 'PROJECT_LEAD' && (
                                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-tighter shrink-0 bg-emerald-50 text-emerald-600">
                                                    Lead
                                                </span>
                                            )}
                                            {online && (
