...
export const GenericDropdown = ({ 
    ...
    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) {
                // If they click inside the portal, don't close. The portal is in document.body
                // So we need to check if the click target is within a portal menu
                const isPortalClick = (e.target as Element).closest('[data-dropdown-portal="true"]');
                if (!isPortalClick) {
                    setIsOpen(false);
                }
            }
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                onClick={toggleOpen}
                className={`flex items-center gap-1.5 px-3 py-2 bg-white border rounded-md text-[13px] font-medium transition-colors shadow-sm h-[38px] whitespace-nowrap shrink-0 ${
                    selected.length > 0
            ...
            {isOpen && triggerRect && createPortal(
                <div 
                    ...
                    className="fixed mt-1 w-48 bg-white border border-slate-200 rounded-lg shadow-lg py-2 z-[9999]"
                    style={{
                            ...
                            {options.map(item => {
                                ...
                                return (
                                    <label key={item} 
                                        ...
                                        <div className={`w-4 h-4 rounded-sm border flex items-center justify-center transition-colors shrink-0 ${isSelected ? 'bg-blue-500 border-blue-500' : 'bg-white border-slate-300 group-hover:border-blue-400'}`}>
