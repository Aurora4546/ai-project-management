...
export const AgileBoard = (): React.ReactElement => {
    ...
    // Custom Edge Scroller to fix @hello-pangea/dnd mobile nested scroll limitations
    useEffect(() => {
        ...
        const scrollLoop = () => {
            if (currentX >= 0 && currentY >= 0) {
                const containers = document.querySelectorAll('.board-scroll-container');
                let activeContainer: Element | null = null;
                
                containers.forEach(container => {
                    const rect = container.getBoundingClientRect();
                    // Add 50px vertical buffer to maintain tracking when item is dragged slightly outside bounds
                    if (currentY >= rect.top - 50 && currentY <= rect.bottom + 50) {
                        activeContainer = container;
                    }
                });

                if (activeContainer) {
                    const container = activeContainer as HTMLElement;
                    const rect = container.getBoundingClientRect();
                    
                    const threshold = 60; // scroll threshold from edge
                    const leftEdge = rect.left + threshold;
                    const rightEdge = rect.right - threshold;
                    const maxSpeed = 15;

                    if (currentX < leftEdge) {
                        const speed = Math.max(2, maxSpeed * (1 - Math.max(0, currentX - rect.left) / threshold));
                        container.scrollLeft -= speed;
                    } else if (currentX > rightEdge) {
                        const speed = Math.max(2, maxSpeed * (1 - Math.max(0, rect.right - currentX) / threshold));
                        container.scrollLeft += speed;
                    }
                }
            }
    ...
    const renderBoardContent = () => {
        if (groupBy === 'NONE') {
            return (
                <div className={`board-scroll-container flex gap-4 md:gap-6 overflow-x-auto pb-4 items-start flex-1 min-h-0 md:snap-none ${isBoardDragging ? '' : 'snap-x snap-mandatory'}`}>
                    {columns.map(col => {
        ...
        return (
            <div className="flex flex-col gap-6 overflow-y-auto pb-8 flex-1 min-h-0 custom-scrollbar pr-2 md:pr-4">
                {groups.map(groupName => {
                    ...
                    return (
                        <div key={groupName} className="flex flex-col bg-white rounded-xl border border-slate-200/80 shadow-sm flex-none">
                            ...
                            <div className={`board-scroll-container flex gap-4 p-4 overflow-x-auto items-start md:snap-none ${isBoardDragging ? '' : 'snap-x snap-mandatory'}`}>
                                {columns
    ...
    return (
        <Layout projectContextName={projectName}>
            <div className="p-3 md:p-8 h-full flex flex-col pt-4 md:pt-6 font-inter overflow-hidden">
